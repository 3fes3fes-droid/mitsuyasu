...
CHANNEL_ID = "UCqdOwN_r041upBwt6QqUV1g"
...
OUT = Path("kana_thumbnail_builder/output")
...
def collect_all():
    ...
    for label, url in TARGETS:
        results.append(collect_one(label, url))

    union = []
    seen = set()
    # Prefer explicit tabs first. The channel root is a safety net.
    ...
    Path(OUT / "video_ids.txt").write_text("\n".join(union) + "\n", encoding="utf-8")
    details = {
        r["label"]: {
...
(() => {
  ...
  addEventListener('keydown', e => {
    ...
    path = OUT / "kana_all_thumbnails.html"
